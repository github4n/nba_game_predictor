...
# range of dates
dates = pd.date_range(start='11/03/1989', end='11/12/2017', freq='D').strftime('%m/%d/%y')
...
nba_ingame_stats = pd.DataFrame(columns = ingame_columns)
nba_meta_stats = pd.DataFrame(columns = meta_columns)
nba_series_stats = pd.DataFrame(columns  = series_columns)
nba_last_meeting_stats = pd.DataFrame(columns = last_meeting_columns)
...
for date in dates:
	try:
		...
		todays_meta_data_df = pd.DataFrame(todays_meta_data, columns=meta_columns)
		...
		todays_series_info_df = pd.DataFrame(todays_series_info,columns = series_columns)
		...
		todays_stats_df = pd.DataFrame(todays_stats,columns=ingame_columns)
		...
		todays_last_meeting_stats_df = pd.DataFrame(todays_last_meeting_stats,columns=last_meeting_columns)
